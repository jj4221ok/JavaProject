...
public class NaverDAO {
	...
	// - 백엔드 서버, odbc를 사용하여 SQL문을 사용할 수 있다. 
	...
	// 항목1. DB접속을 위한 메소드 connect()
	public void connect() {
		conn = DBCon.DBConnect();
		// conn에 DBCon클래스의 DBConnect()메소드의 
		// 리턴값(con)을 저장한다. 
	}
	...
	public boolean idCheck(String dId, String dPw) {
		...
		try {
			...
			// while(rs.next())
			// rs의 결과값이 1개 이기 때문에 while이 아닌 if를 사용
			if(rs.next()) {
				checkResult = true;
			} else {
				checkResult = false;
	...
	// 항복6. 회원정보를 삭제하는 메소드 memberDelete()
